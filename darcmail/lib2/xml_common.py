...
def encode_for_xml (s):
  return dmc.encode_for_character_set(s, 'utf8', 'xml')
...
#####################################################################
def cdata (s):
#  return '<![CDATA[' + encode_for_xml(s) + ']]>'
  return '<![CDATA[' + string.replace(s, ']]', '&#093;&#093;') + ']]>'
...
class XMLCommon ():
  ...
  def pr (self, s):
    if self.indent_level > 0:
      ...
      if self.fh:
        try:
          self.fh.write(spaces[0:i] + s)
        except UnicodeEncodeError:
          self.fh.write(spaces[0:i] + encode_for_xml(s))
      else:
        try:
          sys.stdout.write(spaces[0:i] + s)
        except UnicodeEncodeError:
          sys.stdout.write(spaces[0:i] + encode_for_xml(s))
    else:
      if self.fh:
        try:
          self.fh.write(s)
        except UnicodeEncodeError:
          self.fh.write(encode_for_xml(s))
      else:
        try:
          sys.stdout.write(s)
        except UnicodeEncodeError:
          sys.stdout.write(encode_for_xml(s))
  ...
  def terminal (self, tag_name, tag_value):
    self.prl('<' + tag_name + '>' + escape_xml(tag_value) + '</' + tag_name + '>')
  ...
  def account_head (self, account_name):
    self.prl('<Account xmlns="http://www.archives.ncdcr.gov/mail-account"' + \
      XMLEOL + '  xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"' + \
      XMLEOL + '  xsi:schemaLocation="http://www.history.ncdcr.gov/SHRAB/' + \
      'ar/emailpreservation/mail-account/mail-account.xsd">')
    self.indent()
    self.terminal('GlobalId', account_name)
#    if self.output_chunksize > 0:
#      self.terminal('Comment', 'This is output file #' + str(self.output_file_count) + \
#        ' for this account')
  ...
  def finish_folder (self, folder_name, eol_chars):
    self.Mbox('./' + folder_name, eol_chars)
    self.exdent()
    self.prl('</Folder>')
  ...
  def XMLWrapped (self, bool):
    self.terminal('XMLWrapped', ('true' if bool else 'false'))
  ...
  ######################################################################
  def ExtBodyContent (self, relpath, localid, is_xml, sha1):
    self.prl('<ExtBodyContent>')
    self.indent()
    self.terminal('RelPath', relpath)
    self.terminal('LocalId', localid)
    self.XMLWrapped(is_xml)
    ## <Eol> is optional
