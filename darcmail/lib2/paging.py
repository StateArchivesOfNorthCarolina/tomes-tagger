...
class PagingInfo ():
  def __init__ (self, total_rows):
    ...
    self.total_pages = self.total_rows / dm_wx.MAX_LIST_SIZE
  ...
  def page_links (self, current_page, parent, page_button_handler):
    ...
    page_sizer.Add((30,0))
    page_sizer.Add(wx.StaticText(parent, id=-1,
        label='page (of ' + str(self.total_pages) + '):', name='page_label'),
        0, wx.ALL, 5)
    if self.current_low > 1:
      b = wx.Button(parent, id=-1, label='<<', name='page_left', size=(40,-1))
    ...
    for p in range(self.current_low, self.current_high+1):
      ...
      b = wx.ToggleButton(parent, id=-1, label=str(p), name='page_' + str(p), size=(w,-1))
      page_sizer.Add(b, 0, wx.ALIGN_LEFT|wx.ALL, 5)
      ...
      if p == self.current_page:
        ...
        b.Refresh()
    if self.current_high < self.total_pages:
      b = wx.Button(parent, id=-1, label='>>', name='page_right', size=(40,-1))
      page_sizer.Add(b, 0, wx.ALIGN_LEFT|wx.ALL, 5)
      b.Bind(wx.EVT_BUTTON, page_button_handler)
    return page_sizer
