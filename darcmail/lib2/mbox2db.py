...
class Mbox2Db ():
  ...
  def __init__ (self, cnx, logf, account_name, account_directory,
      account_id, folder_name, folder_directory, external_size,
...
#    lines.append('multipart parts: ' "{:,}".format(
#        dba.count_multipart (self.cnx, account)))
#    lines.append('parts of parts: ' "{:,}".format(
#        dba.count_grandchild_parts (self.cnx, account)))
  ...
  def write_log (self):
    ...
    self.logf.write('total messages loaded: ' + str(self.total_msg_count) + EOL)
...
#    for (tag, value) in non_predef.items():
...
#        self.warning('skipping tag ' +  tag + '; empty value')
      ...
      if ltag not in self.lc_original_lookup.keys():
        continue
#        new_tag_id = dba.insert_tag(self.cnx, tag)
#        if new_tag_id:
#          self.lc_original_lookup[tag.lower()] = tag
#          self.original_tag2id[tag]            = new_tag_id
#          self.id2xml_tag[new_tag_id]          = ''
##          self.warning('inserted new tag ' + tag)
  ...
  ######################################################################
  def do_mbox_folder (self, folder_id, mbox_file):
    mb = mailbox.mbox(mbox_file, create=False)
    cs = content_store.ContentStore(self.cnx, folder_id,
        self.folder_directory, self.external_size,
        self.external_subdir_levels, self.xml_wrap)
    for i in range(len(mb)):
      ...
      try:
        self.do_message(cs, folder_id, em, eol, sha1_digest)
        self.cnx.commit()
...
#          pass
# updating by estimation often ends badly... flow control the dialog creator
# can get messed up
  ...
  def walk_account_tree (self, relative_path, path):
    ...
    for f in files:
      if os.path.isdir(os.path.join(path, f)):
        ...
        mbox_filename = os.path.join(path, f)
        if not check_file_for_reading(mbox_filename):
          sys.exit(0)
        folder_id  = dba.get_folder_id(self.cnx, self.account_id,
            folder_name, relative_path, make=True)
        self.folder_directory = path
        self.do_mbox_folder(folder_id, mbox_filename)
