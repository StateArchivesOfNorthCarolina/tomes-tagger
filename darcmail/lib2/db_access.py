...
######################################################################
def set_client_character_set (cnx, cs):
  if cs == 'iso-8859-1':
    cs = 'latin1'
  cursor = cnx.cursor()
  cursor.execute("SET CHARACTER_SET_CLIENT='" + cs + "'", None)
  for row in cursor:
    pass
  cursor.close()
  cursor = cnx.cursor()
  cursor.execute("SET CHARACTER_SET_RESULTS='" + cs + "'", None)
  for row in cursor:
    pass
  cursor.close()

######################################################################
def connect (username, passwd, host, database):
  cnx = mysql.connector.connect(user=username, password=passwd,
      host=host, database=database)
#  try:
#    set_client_character_set(cnx, CLIENT_CHARACTER_SET)
...
def get_folder_names_for_account (cnx, account_id):
  folder_names = set()
  query = "select folder_name from folder where account_id=" + str(account_id);
  cursor = cnx.cursor()
  cursor.execute(query, None)
  for row in cursor:
...
def get_folder_info (cnx, folder_id):
  (folder_name, relative_path, account_id, account_name) = (None, None, None, None)
  query = "select f.folder_name, f.relative_path, a.id, a.account_name " + \
      "from account a, folder f where " + \
      "f.account_id=a.id and f.id=" + str(folder_id)
  cursor = cnx.cursor()
  cursor.execute(query, None)
  for row in cursor:
    (folder_name, relative_path, account_id, account_name) = row
  cursor.close()
  return (folder_name, relative_path, account_id, account_name)
...
#    print 'escape_sql:', str(e)
    s0 = dmc.encode_for_character_set(s, CHARACTER_SET, 'backslash')
  s0 = re.sub("\\\\", "\\\\\\\\", s0)
  s1 = re.sub("'", "\\\\'", s0)
  return s1
...
######################################################################
def create_tag_dictionary (cnx):
  tag2id = {}
  query = "select id,original_name from tag where original_name != ''"
  cursor = cnx.cursor()
  cursor.execute(query, None)
  for row in cursor:
    (tag_id, name) = row
...
def delete_folder_parts (cnx, folder_id):
  query = "delete p from part p, message m " + \
...
def delete_folder_replies (cnx, folder_id):
  query = "delete r from replyto r " + \
      "INNER JOIN message m " + \
      "ON r.replying_id=m.id " + \
...
def delete_folder_part_headers (cnx, folder_id):
  query = "delete ph from part_header ph, part p, message m " + \
    "where ph.part_id=p.id and " + \
...
def delete_folder_message_headers (cnx, folder_id):
  query = "delete mh from message_header mh, message m " + \
    "where mh.message_id=m.id and " + \
    "m.folder_id=" + str(folder_id)
  cursor = cnx.cursor()
  cursor.execute(query, None)
  for row in cursor:
...
def delete_folder_message_address (cnx, folder_id):
  query = "delete ma from message_address ma, message m " + \
    "where ma.message_id=m.id and " + \
    "m.folder_id=" + str(folder_id)
  cursor = cnx.cursor()
  cursor.execute(query, None)
  for row in cursor:
...
def existing_external_content (cnx, sha1, content_length,
  ...
  for row in cursor:
    if external_content_id:
      print 'multiple external_content_ids match id=', external_content_id
...
def set_is_attachment (cnx, part_id, is_attachment):
  query = "update part set is_attachment=" + ('1' if is_attachment else '0') + \
...
def insert_internal_content (cnx, content, original_file_name, folder_id,
    content_length, content_sha1):
  query = 'insert into internal_content(content_text,original_file_name,folder_id,' + \
    'content_length,content_sha1) ' + \
    'values (' + \
    "'" + escape_sql(content) + "'," + \
    ("'" + escape_sql(original_file_name) + "'" if original_file_name else "null") + "," + \
    str(folder_id) + "," + \
    str(content_length) + "," + \
    "'" + content_sha1 + "'" + \
    ");"
...
def insert_message_address(cnx, message_id, tag_id, address_id, name_id):
  query = "insert into message_address(message_id,tag_id,address_id,name_id) " + \
...
######################################################################
def get_child_parts (cnx, message_id, parent_sequence_id):
  part_list  = []
  query = "select id, sequence_id from part " + \
    "where message_id=" + str(message_id) + " " + \
    "and parent_sequence_id=" + str(parent_sequence_id)
  cursor = cnx.cursor()
  cursor.execute(query, None)
  for row in cursor:
...
def get_top_part (cnx, message_id):
  part_id = None
  query = "select id from part where message_id=" + str(message_id) + " " + \
    "and sequence_id=1"
  cursor = cnx.cursor()
  cursor.execute(query, None)
  for row in cursor:
...
######################################################################
#def get_parts (cnx, message_id):
#  part_list = []
#  query = "select id, sequence_id, parent_sequence_id " + \
#    "from part where message_id=" + str(message_id)
#  cursor = cnx.cursor()
#  cursor.execute(query, None)
#  for row in cursor:
#    part_list.append(row)
...
def get_folder_names (cnx, account_id):
  folder_list = []
  query = "select id, folder_name from folder where account_id=" + str(account_id)
  cursor = cnx.cursor()
  cursor.execute(query, None)
  for row in cursor:
...
def get_message_headers (cnx, message_id):
  ...
  query = "select t.original_name,t.xml_name,h.header_value from " + \
      "tag t,message_header h where h.message_id=" + str(message_id) + " and " + \
      "h.tag_id=t.id"
  cursor = cnx.cursor()
  cursor.execute(query, None)
...
######################################################################
def get_folder_rel_path (cnx, folder_id):
  rel_path = None
  query = "select relative_path from folder where id=" + str(folder_id)
  cursor = cnx.cursor()
  cursor.execute(query, None)
  for row in cursor:
    rel_path = row[0]
...
######################################################################
def insert_folder (cnx, account_id, folder_name, relative_path):
  folder_id = None
  query = "insert into folder(account_id,folder_name,relative_path) values" + \
...
######################################################################
def get_folder_id (cnx, account_id, folder_name, relative_path, make=False):
  folder_id = None
  ...
  if not folder_id:
    if not make:
      ...
      return insert_folder(cnx, account_id, folder_name, relative_path)
  return folder_id
...
######################################################################
def search_message_for_address (cnx, account_id, address_id, address_type):
  message_info = []
  query = "select m.id, m.date_time, date(m.date_time), mh.header_value " + \
      "from message m, folder f, message_header mh, tag t, message_address ma where " + \
      "m.folder_id=f.id and " + \
...
def lookup_account_directory (cnx, account_directory):
  ...
  query = "select id, account_name, account_directory from account " + \
      "where account_directory = '" + account_directory + "' " + \
      "order by account_name"
  cursor = cnx.cursor()
  cursor.execute(query, None)
  for row in cursor:
...
class DataBaseConnection:
  ...
  def __init__ (self, host, username, passwd, database):
    self.cnx = None
    self.cnx = mysql.connector.connect(user=username, password=passwd,
      host=host, database=database)
    if self.cnx == None:
      print 'Cannot connect to ' + host + ' with user=' + username + \
          ', database=' + database
      sys.exit(0)
...
class DbFolder ():
  def __init__ (self, cnx, folder_id):
    ...
    self.start_date    = ''
    self.end_date      = ''
    (self.folder_name, self.relative_path, self.account_id, self.account_name) = \
...
class DbMessage ():
  ...
  def fetch_body (self, cnx, message_id):
    if False:
      query = "select p.sequence_id, m.folder_id, p.content_length, ph1.header_value, " + \
      "ph2.header_value, " + \
...
# for a viewable content-type
      if content_length > 0 and content_type[0:10] != 'multipart/':
        self.body_info.append(DbContent(cnx, content_id, folder_id, sequence_id, content_length,
            content_type, transfer_encoding, is_attachment, is_internal, content_ident,
            xml_wrapped, charset))
