...
class AddressList (wx.Panel):
  ...
  def make_list (self, parent):
    ...
    col2 = wx.StaticText(parent, id=-1, label='Address')
    col3 = wx.StaticText(parent, id=-1, label='Name')
    ...
    for i in range(self.pi.current_lo_i, self.pi.current_hi_i):
      ...
      col2 = wx.StaticText(parent, id=-1, name='', label=str(address))
      col3 = wx.StaticText(parent, id=-1, name='', label=str(name))
  ...
  def OnAddressIdClick(self, event):
    ...
    address_info.AddressInfo('', self.browse, self.results_notebook,
        self.cnx, dba.DbAddress(self.cnx, self.browse.current_account_id,
        aid, None))
