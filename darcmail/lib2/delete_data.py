#!/usr/bin/env python
######################################################################
## $Source: /Users/Carl/cvsroot/DArcMail/src/python/delete_data.py,v $
## $Revision: 1.1 $
...
class DeleteData ():
  ...
  def __init__ (self, cnx, account_name, account_directory, account_id,
      folder_name, remove_external):
    self.cnx               = cnx
    self.account_name      = account_name
    self.account_directory = account_directory
    self.account_id        = account_id
    self.folder_name       = folder_name
    self.remove_external   = remove_external
  ...
  def delete_subdirs(self, curdir):
    ...
    if m and os.path.isdir(curdir):
      ...
      if len(os.listdir(curdir)) == 0:
        try:
          os.chmod(curdir, 0666)
          os.rmdir(curdir)
        except:
          print "Unexpected error while attempting to rmdir " + curdir, sys.exc_info()[0]
  ...
  def delete_external_content (self, dbf):
    if self.remove_external:
      folder_directory = os.path.abspath(
          os.path.join(self.account_directory, dbf.relative_path))
  ...
  def delete_addresses_and_names (self):
    ...
    query = "delete an from address_name an, " + \
        "(select a.id from address a LEFT JOIN message_address ma on " + \
        "a.id=ma.address_id where ma.address_id is null) as xa " + \
        "where xa.id=an.address_id"
    ...
    # delete from address_name where name_id is not used in message_address
    query = "delete an from address_name an, " + \
        "(select n.id from name n LEFT JOIN message_address ma on " + \
        "n.id=ma.name_id where ma.name_id is null) as xn " + \
        "where xn.id=an.name_id"
    cursor = self.cnx.cursor()
    cursor.execute(query, None)
    cursor.close()

    # delete from address where id is not used in message_address
    query = "delete a from address a left join message_address ma on " + \
    ...
    query = "delete n from name n left join message_address ma on " + \
