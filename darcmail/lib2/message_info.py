...
class MessageInfo (wx.Panel):
  ...
  ####################################################################
  def __init__ (self, name, browse, results_notebook, cnx, data, search_term=None):
    ...
    sizer.Add((FRAME_WIDTH, 10))
    aname = wx.StaticText(self, id=-1, label='Message Content')
  ...
  def OnBodyPartClick (self, event):
    ...
    if action == 'show path':
      ...
      fld_dir = dba.get_folder_rel_path(self.cnx, self.msg.folder_id)
      path = os.path.abspath(
          os.path.join(acc_dir, fld_dir, content_data.stored_file_name))
  ...
  def download_content (self, content_data):
    ...
    if content_data.is_internal:
      ...
      acc_dir = dba.get_account_directory(self.cnx, self.msg.account_id)
      fld_dir = dba.get_folder_rel_path(self.cnx, self.msg.folder_id)
      path = os.path.join(acc_dir, fld_dir, content_data.stored_file_name)
      input = open(path)
  ...
  def body_part_list (self):
    acc_dir = dba.get_account_directory(self.cnx, self.msg.account_id)
    fld_dir = os.path.join(acc_dir,
        dba.get_folder_rel_path(self.cnx, self.msg.folder_id))
